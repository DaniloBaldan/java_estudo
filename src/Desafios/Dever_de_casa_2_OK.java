//package Desafios;

public class Dever_de_casa_2_OK {

    //1 - Crie um programa que realize a média de duas notas decimais e exiba o resultado (OK)
//    public static void main(String[] args) {
//        double nota1 = 10;
//        double nota2 = 1;
//        double media = (nota1 + nota2) / 2;
//        System.out.println("Média " + media);

    //2 - Declare uma variável do tipo double e uma variável do tipo int. Faça o casting da variável double para int e imprima o resultado.
//    public static void main(String[] args) {
//        double numeroDouble = 15.7;
//        int numeroInteiro = (int) numeroDouble;
//        System.out.println("Casting de double para int: " + + numeroInteiro);

    //3 - Declare uma variável do tipo char (letra) e uma variável do tipo String (palavra). Atribua valores a essas variáveis e concatene-as em uma mensagem.
//    public static void main(String[] args) {
//        char letra = 'A';
//        String palavra = "Danilo";
//        String mensagem = "A letra é " + letra + " e a palavra é " + palavra;
//        System.out.println(mensagem);

    //4 - Declare uma variável do tipo double precoProduto e uma variável do tipo int (quantidade). Calcule o valor total multiplicando
    //o preço do produto pela quantidade e apresente o resultado em uma mensagem.
//    public static void main(String[] args) {
//        double precoPrdouto = 15.88;
//        int quantidade = 15;
//
//        double total = precoPrdouto * quantidade;
//        String mensagem = "O valor total da compra é R$" + total;
//
//        System.out.println(mensagem);
//    }

    //5 - Declare uma variável do tipo double valorEmDolares. Atribua um valor em dólares a essa variável. Considere que o valor de 1 dólar é equivalente
    //a 4.94 reais. Realize a conversão do valor em dólares para reais e imprima o resultado formatado.
//    public static void main(String[] args) {
//        double valorEmDolares = 6.55;
//        double taxaDoDolar = 4.94;
//
//        double conversaoDoDolar = valorEmDolares * taxaDoDolar;
//        String mensagem = "O valor do dolar é: " + valorEmDolares;
//        System.out.println(mensagem);
//
//    }

    //6- Declare uma variável do tipo double precoOriginal. Atribua um valor em reais a essa variável, representando o preço original de um produto.
    //Em seguida, declare uma variável do tipo double percentualDesconto e atribua um valor percentual de desconto ao produto (por exemplo, 10 para 10%).
    //Calcule o valor do desconto em reais, aplique-o ao preço original e imprima o novo preço com desconto.
//    public static void main(String[] args) {
//        double precoOriginal = 300;
//        double percentualDesconto = 10;
//
//        double valorDoDesconto = (percentualDesconto/100) * precoOriginal;
//        double novoPreco = precoOriginal - valorDoDesconto;
//
//        System.out.println("Preço original: R$ " + precoOriginal);
//        System.out.println("Deconto: R$:" + valorDoDesconto);
//        System.out.println("Novo Preço com deconto R$: " + novoPreco);
//    }
  }





